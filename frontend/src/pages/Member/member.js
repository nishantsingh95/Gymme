...
const Member = () => {
  ...
  const fetchData = async (skip, limits) => {
    await axios
      .get(
        `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/members/all-member?skip=${skip}&limit=${limits}`,
  ...
  const handleSearchData = async () => {
    if (search !== "") {
      ...
      await axios
        .get(
          `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/members/searched-members?searchTerm=${search}`,
          { withCredentials: true }
        )
        .then((response) => {
          console.log(response);
  ...
  return (
    <div className="flex-1 text-black p-5 h-screen overflow-y-auto">
      {/* block for banner */}
      ...
      <div className="mt-5 text-xl flex justify-between text-slate-900">
        ...
        {!isSearchModeOn ? (
          <div className="flex gap-5">
            ...
            <div
              className={`w-8 h-8 cursor-pointer border-2 flex items-center justify-center hover:text-white hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 ${currentPage === 1 ? "bg-grey-200 text-grey-400" : null
                }`}
              onClick={() => {
                handlePrev();
              }}
            ...
            <div
              className={`w-8 h-8 cursor-pointer border-2 flex items-center justify-center hover:text-white hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 ${currentPage === noOfPage ? "bg-grey-200 text-grey-400" : null
                }`}
              onClick={() => {
                handleNext();
              }}
